...
function buildRouting (options) {
  ...
  function route ({ options, isFastify }) {
    ...
    function addNewRoute ({ path, prefixing = false, isFastify = false }) {
      ...
      // If the user registers their own OPTIONS route for a path that
      // already has an automatically exposed OPTIONS route, the user
      // route takes precedence and the generated one is removed.
      if (globalExposeOptionsRoutes && isOptionsRoute && !isFastify) {
        const exposedOptionsRoute = router.findRoute('OPTIONS', opts.url, constraints)
        if (exposedOptionsRoute !== null && exposedOptionsRoute.store[kRouteByFastify]) {
      ...
      // register the automatic OPTIONS route last, so that every method
      // already registered for this path is taken into account
      if (globalExposeOptionsRoutes && !isOptionsRoute && router.findRoute('OPTIONS', opts.url, constraints) === null) {
        prepareRoute.call(this, {
          method: 'OPTIONS',
          url: path,
          options: {
            constraints: opts.constraints,
            handler: buildOptionsHandler(url, constraints)
          },
          isFastify: true
        })
      }
  ...
  // Builds the handler used by the automatically exposed OPTIONS routes.
  // The `Allow` header is computed when the request is received so that
  // it always reflects the methods currently registered for the path.
  function buildOptionsHandler (url, constraints) {
    if (ignoreDuplicateSlashes) {
      url = FindMyWay.removeDuplicateSlashes(url)
    }
    if (ignoreTrailingSlash) {
      url = FindMyWay.trimLastSlash(url)
    }

    return function optionsHandler (request, reply) {
      const supportedMethods = this[kSupportedHTTPMethods]
      const allow = []
      for (const method of supportedMethods.bodyless) {
        if (router.hasRoute(method, url, constraints)) {
          allow.push(method)
        }
      }
      for (const method of supportedMethods.bodywith) {
        if (router.hasRoute(method, url, constraints)) {
          allow.push(method)
        }
      }

...
function buildRouting (options) {
  ...
  return {
    ...
    setup (options, fastifyArgs) {
      ...
      globalExposeOptionsRoutes = options.exposeOptionsRoutes
  ...
  function route ({ options, isFastify }) {
    ...
    if (Array.isArray(opts.method)) {
      for (let i = 0; i < opts.method.length; ++i) {
        ...
        isOptionsRoute = opts.method.includes('OPTIONS')
      }
    } else {
      ...
      isOptionsRoute = opts.method === 'OPTIONS'
    }
    ...
    function addNewRoute ({ path, prefixing = false, isFastify = false }) {
      ...
      // If the user registers an OPTIONS route for a path where an OPTIONS
      // route was automatically exposed, the user route takes precedence
      // and the generated one is removed before registering the new route.
      if (globalExposeOptionsRoutes && isOptionsRoute) {
        const exposedOptionsRoute = router.findRoute('OPTIONS', opts.url, constraints)
        if (exposedOptionsRoute !== null && exposedOptionsRoute.store[kRouteByFastify] === true) {
          router.off('OPTIONS', opts.url, constraints)
        }
      }
      ...
      // register OPTIONS route in sync
      // we must place it after the `this.after`

      if (globalExposeOptionsRoutes && !isOptionsRoute) {
        const hasOPTIONSHandler = router.findRoute('OPTIONS', opts.url, constraints) !== null
        if (!hasOPTIONSHandler) {
          const optionsRouteOpts = {
            handler: optionsRouteHandler,
            constraints: opts.constraints,
            prefixTrailingSlash: opts.prefixTrailingSlash
          }
          prepareRoute.call(this, { method: 'OPTIONS', url: path, options: optionsRouteOpts, isFastify: true })
        }
      }
    }
  }

  // Handler of the automatically exposed OPTIONS routes. It answers 204 with
  // the Allow header listing every method currently available on the
  // requested path, computed at request time so that it always reflects the
  // routes registered on the router (including the HEAD routes exposed by
  // the exposeHeadRoutes option).
  function optionsRouteHandler (req, reply) {
    const constraints = router.constrainer.deriveConstraints(req)
    const allowedMethods = ['OPTIONS']

    for (const method of this[kSupportedHTTPMethods].bodyless) {
      if (router.find(method, req.url, constraints) !== null) {
        allowedMethods.push(method)
      }
    }

    for (const method of this[kSupportedHTTPMethods].bodywith) {
      if (method !== 'OPTIONS' && router.find(method, req.url, constraints) !== null) {
        allowedMethods.push(method)
      }
    }
